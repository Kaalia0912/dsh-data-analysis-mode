...
- 脚本位于本技能目录下：scripts/analyze.py；在技能目录内执行 python scripts/analyze.py ...（或写绝对路径）。
- 数据文件路径：DSH 会话中直接用工作区内的相对/绝对路径，无需 /mnt 前缀。
- 输出与缓存写到工作区（默认当前目录或其下），自动创建的缓存目录为 .data-analysis-cache。
...
python scripts/analyze.py \
  ...
  --output-file filtered-results.csv
```
...
- For large results, export to file and 把导出文件的实际路径告知用户（文件已写入工作区）
